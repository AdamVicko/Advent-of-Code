...
$input = file_get_contents('Day2.txt'); // ucitaj mi sve iz datoteke kao string

$lines = explode("\n", $input);// za svaki novi red napravi novi string (razdvaja string na odredeni znak)

$totalwrapp=0;
$totalribbon=0;

foreach($lines as $line) { //svaku zasebnu lines spremi i citaj kao line

    if(!$line){ // ako je linija prazna
        continue;
    }

        $dimensions = explode('x', $line); // spremi u dimensions svaki dio line dok ih x ne prekine(x ih razdvaja)
                                        //explode prepoznaje ostatke znaci do prvog x-a izmedu prvog i drugog x-a i poslije drugog x-a itd itd...
        $l = $dimensions[0]; //length
        $w = $dimensions[1]; //width
        $h = $dimensions[2]; //height

        $sides = [2 * ($l * $w) , 2 * ($w * $h) , 2 * ($l * $h)];  // spremam u array da bi pomocu ugradene funkcije min mogo lakse nac najmanju stranicu

            //WRAPP
            $smallestside = (min($sides) ) / 2; //najmanju stranicu 
            

            $sidesumwrapp = $sides[0] + $sides[1] + $sides[2]; //cijelokupna povrsina za jedan poklon

            $totalwrapp += ($smallestside + $sidesumwrapp); //sveukupni wrappingpaper

            //RIBBON


            $ribbonwrap = 2*($l + $w + $h - max($l, $w, $h)); // 
            $ribbonbow = $l*$w*$h;

            $totalribbon += ($ribbonbow + $ribbonwrap);
}

echo 'Total feet of wrapping paper needed is ' . $totalwrapp . '<br>' . 'Total feet of ribbon required is ' . $totalribbon;
